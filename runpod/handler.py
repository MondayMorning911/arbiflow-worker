...
async def handler(job):
    ...
    try:
        ...
        if task == "ai_subs":
            ...
            MIN_BLOCK_DURATION = 45   # Минимальный блок 45 секунд (Thematic Chunking)
            MAX_BLOCK_DURATION = 60   # Максимальный блок 60 секунд
            PAUSE_THRESHOLD = 1.0     # Пауза 1+ секунда считается сменой сцены
...
Твоя задача — найти максимум сильных тематических арок (Thematic Arc Mode).

Это этап генерации кандидатов (THEMATIC RECALL).
...
2. Ищи "смысловые узлы" со структурой:
   - Завязка (Hook): Обозначение конкретной темы (вопрос, тезис, провокация).
   - Развитие (Expansion): Аргументация, личный опыт, объяснение.
   - Завершение (Resolution): Логический вывод или яркая финальная точка.
3. Игнорируй:
   - трейлерные нарезки
   - анонсы “в этом выпуске”
   - рекламные вставки
   - пустые диалоги ("ага", "ну да", "согласен")
4. Для каждого кандидата проверь:
   ✓ Начинается с речи (не музыка)
   ✓ Нет пауз >3 секунд
   ✓ Длительность 40–180 секунд (если тема глубокая — берем до 3 минут)
   ✓ Мысль полностью раскрыта (самодостаточность)
   ✓ Можно понять без контекста всего видео
...
10 — сильный шок / конфликт / мощный инсайт
8–9 — сильная эмоция или глубокое раскрытие темы
7 — хороший удерживающий тематический блок
<7 — не возвращать
...
{{
  "candidates": [
    {{
      ...
      "scene_topic": "Коротко о чем сцена (тема)"
    }}
  ]
}}
"""
            ...
            for s in all_scenes:
                if 'start_time' in s and 'end_time' in s:
                    try:
                        ...
                        # Расширенный диапазон 40-180 сек
                        if s['end_time'] - s['start_time'] >= 40:
            ...
            print(f"🎯 [ArbiFlow Worker]: Found {len(valid_scenes)} non-overlapping candidates. Running global selection (Thematic Arc Mode)...", flush=True)
            ...
            global_prompt = f"""
Ты — главный редактор тематических видео.
Твоя задача — из списка кандидатов выбрать ровно 10
самых сильных и разнообразных тематических историй.

Это финальный этап (GLOBAL SELECTION).

==================================================
ТРЕБОВАНИЯ:

1. Ровно 10 сцен (или меньше, если видео короткое, но старайся найти 10).
2. Без пересечений по времени.
3. РАЗНООБРАЗИЕ: если несколько сцен на одну тему (например, про сахар), выбери только ОДНУ самую глубокую и качественную.
4. Только сцены с полной драматургией: завязка → развитие → вывод.
5. Сцены должны быть самодостаточными.
6. Генерируй точное описание и заголовок на основе реально сказанных слов.
...
4. Разнообразие тем по всему хронометражу
...
{{
  "final_scenes": [
    {{
      ...
      "viral_reason": "Почему эта история важна (до 10 слов)"
    }}
  ]
}}
"""
